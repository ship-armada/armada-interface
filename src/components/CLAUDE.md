...
| `WalletMenu/` | Wallet side-panel (pill → slide-out) — address, hide toggle, actions, and a **per-chain** USDC breakdown (one row per chain holding USDC; `buildWalletChainBalances`), since a shield is per-chain and a combined total misleads (#8). Chain-agnostic "Shield your USDC" opens the modal whose Network picker handles selection |
