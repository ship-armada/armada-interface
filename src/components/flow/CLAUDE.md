...
| `ErrorStep` | Icon + headline + message + Try Again (disabled when `onRetry` omitted) + optional View Details. Copy comes from `lib/tx/errorCopy.ts` (shared with the ActivityReceipt). An optional `remedy` (an in-app fix, e.g. "Merge notes" from `useMergeNotes().remedyFor(record)`) becomes the primary button and moves Try Again / Start over to the secondary slot. |
