...
| `TxRow` | Compact row — title + amount + status + relative time, with an optional stage-copy sub-line + progress strip. V1 Phase 9 added the `transfer-shielded-received` kind: incoming privately-paid USDC rendered with the Inbox glyph + green inflow amount; synthesized from chain by useHistoryRecovery / useIncomingTransferDetector. A `consolidate` (merge notes) row uses the Combine glyph and shows its fee — it moves no value (`lib/tx/headlineAmount`). |
...
| `TxActions` | Per-record CTAs (retry / cancel / stop-tracking) driven by the executor's `canRetry`/`canCancel` predicates. |
| `processing/` | The redesigned in-flight "processing" surface — `TxProgressCard` (tick-ring around the USDC logo) + `TransactionProgressDisclosure` timeline + `TxProcessingLayout`; view model in `processingView.ts` / copy in `processingCopy.ts`. Rendered by `flow/ProgressStep` off the live record. |
