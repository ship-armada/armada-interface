...
| `SendInputStep` | Amount step. `DepositAmountCard` with the chain rendered **statically** (chosen on the recipient step). Gates Review on the amount only. A private send's fee caption reads "Estimating fees…" until the plan prices the amount (never the one-proof quote first), and "—" when the planner refuses it (Review's fee and Total rows too). |
