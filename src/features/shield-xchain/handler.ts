...
async function runBuildProof(
  ...
  // The cross-chain note-build is identical to the hub shield — only the CCTP wrapping below differs.
  const request = await createShieldRequestSdk(shieldedAddress, shieldValue, hubUsdcAddress, shieldPrivateKey)
  if (ctx.signal.aborted) throw new Error('cancelled')
...
async function buildGaslessXchainArtifacts(
  ...
  const feeNote = await createShieldRequestSdk(record.meta.broadcasterShieldedAddress, record.meta.feeAmount, hubUsdcAddress, feeShieldPrivateKey)
  if (ctx.signal.aborted) throw new Error('cancelled')
...
async function runWaitForDelivery(
  ...
  // SDK at shield-request time, so unique with overwhelming probability.
