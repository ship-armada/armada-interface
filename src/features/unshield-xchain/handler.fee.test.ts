// ABOUTME: Tests for the unshield-xchain handler's build-proof fee handling: it plans at the per-proof fee, caps the build
...
vi.mock('@/lib/shielded/unshield-xchain-sdk', () => ({ buildXchainUnshieldSdk: hoisted.build }))
...
vi.mock('@/config/deployments', () => ({
  loadDeployments: async () => ({ hub: { contracts: { privacyPool: '0xpool' } }, clients: [{ chainId: 31338 }] }),
...
type Rec = TxRecord<'unshield-xchain'>
...
describe('unshieldXchainHandler — build-proof fee', () => {
  it('plans at the per-proof fee and caps the build at the reviewed total', async () => {
    const { ctx } = makeCtx()
    await unshieldXchainHandler.run(buildRecord(), ctx)
  ...
  it('plans a record without a per-proof fee at its stored fee (records from before review-time planning)', async () => {
    const { ctx } = makeCtx()
    await unshieldXchainHandler.run(buildRecord({ broadcasterFeeAmount: 20_000n, broadcasterFeePerProof: undefined }), ctx)
    expect(hoisted.build).toHaveBeenCalledWith(
      expect.objectContaining({
        broadcasterFee: { amount: 20_000n, recipientAddress: '0zk_relayer' },
        maxTotalFee: 20_000n,
