...
export async function createWallet(_passphrase: string): Promise<{ mnemonic: string; railgunAddress: string }> {
...
export async function unlockWallet(_passphrase: string): Promise<ShieldedWalletState> {
  throw new Error('railgun.wallet.unlockWallet: not implemented (scaffold).')
}

/** Drop in-memory keys (does NOT delete the encrypted blob). */
export function lockWallet(): void {
  // no-op stub
}

/** Settings → Reset wallet: irreversibly delete the encrypted blob + all derived state. */
