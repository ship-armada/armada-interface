...
| `transfer-sdk.ts` | Private-send builder: `buildTransferSdk` plans (possibly split into several proofs for a fragmented wallet), runs one batched `preflight(plans)`, `proveAll(plans)` (one signing batch, progress across all proofs), and combines everything into one `transact([...])`. Returns `totalFee` (sum of the plans' fee notes) and refuses with `TransferFeeIncreasedError` if that exceeds the reviewed `maxTotalFee`. `planTransferFee` / `maxTransferAmount` price a send without proving, for review time (`hooks/useTransferFeePlan`). | Working |
| `transfer-fee-error.ts` | Leaf module (no imports): `TransferFeeIncreasedError`, so `lib/tx/errors` can classify it (→ `FEE_EXPIRED`, start over) without pulling in the SDK. | Working |
