...
| `artifacts.ts` | Warms the common ZK circuit shapes into the `artifactGetter` registry on mount (via `circuitFetch`), so the first proof doesn't pay a fetch. Heavy/rare shapes lazy-load on demand. | Working |
| `circuitFetch.ts` | Fetches a shape's `{zkey,wasm,vkey}` from the configured artifact host (`VITE_ARTIFACTS_BASE_URL`, default same-origin `/artifacts`), SHA-256-verifies each against the committed `config/circuits.manifest.json`, de-dupes concurrent fetches, and populates the registry. 404 / hash-mismatch → hard error, no IPFS fallback (#6). | Working |
...
| `sdk-read.ts` | The `@armada/sdk` shielded read path — a persistent IndexedDB-backed SDK instance (`ensureInstance`/`closeSdkRead`). Sole source of all shielded balance + history reads. **Sync vs read are split**: `refreshShieldedBalances` is the ONLY syncer (`wallet.sync()`, driven by the 15s poll + post-tx refresh + initial unlock); `readSdkUsdcBalance` / `readSdkYieldShares` / `readSdkHistory` read the current scan state WITHOUT syncing (called from balance-bus event handlers, which fire because a sync just completed — re-syncing there is circular). `readSdkNoteCounts` (spendable notes per mergeable token) + `mergeTokenAddress` resolve the merge flow's inputs. | Working |
| `sdk-telemetry.ts` | Leaf module — the `TelemetrySink` handed to `createArmadaSdk`. Maps the SDK's `sync.quicksync` event → `track('sdk.quicksync')` so an operator can confirm a configured indexer (watcher) is actually serving a root-verified batch (`served`, `tailCovered` on tail-lag) vs being rejected (`root-mismatch-fallback`). On fallback it also forwards the classified `reason` (indexer-http-error / schema-mismatch / root-mismatch / position-gap / unknown) + HTTP `status`, so the discard cause is observable. Emits nothing on RPC-only syncs. | Working |
| `history.ts` | Chain-driven history recovery — `runHistoryScan` syncs the `@armada/sdk` wallet, maps each `HistoryEntry` → `TxRecord` (`historyEntryToTxRecord`, pure), backfills block timestamps, and returns a checkpoint candidate. `syntheticTxId` encodes deterministic ids. Yield ops classified natively by the SDK. A spend tagged `consolidation` in its self-metadata (`selfMetadata.ts`, wire `m`) maps back to a `consolidate` record — a merge has no recipient output, so the SDK alone would recover it as an anonymous `transfer-sent`. | Working |
...
| `transfer-sdk.ts` | Private-send builder: `buildTransferSdk` plans (possibly split into several proofs for a fragmented wallet), runs one batched `preflight(plans)`, `proveAll(plans)` (one signing batch, progress across all proofs), and combines everything into one `transact([...])`. Returns `totalFee` (sum of the plans' fee notes) and refuses with `SpendFeeIncreasedError` if that exceeds the reviewed `maxTotalFee`. `planTransferFee` / `maxTransferAmount` price a send without proving, for review time (`hooks/useTransferFeePlan`); `maxTransferAmount` is the SDK's `wallet.maxTransferAmount` (the planner's own rules: one tree, one fee per proof, the 4-proof cap), so it can be less than balance minus a fee. | Working |
| `consolidate-sdk.ts` | Consolidation builder (armada-sdk #98): `buildConsolidateSdk` → `wallet.consolidate` (one token; old-tree notes first, then smallest; up to 4 proofs, one USDC per-proof fee each) → reviewed-fee cap (`SpendFeeIncreasedError`) → batched preflight → stash → `proveAll` → one `transact([...])`. Returns `totalFee` + the token's `notesMerged` / `notesCreated`. `previewConsolidation` prices it without proving and, given the blocked spend, dry-runs it on the post-merge wallet (`blockedWillWork`). `checkSpendPlans(request)` dry-runs a spend against the current notes and returns the fee it plans (`hooks/useSpendCheck`: review-time fee + fragmentation check). | Working |
| `merge-intent.ts` | What the merge-notes flow opens with: `MergeToken` (`usdc` / `shares`), `mergeIntentFromRecord` (the token + blocked spend of a record that failed for fragmentation), `blockedSpendRequest` (the SDK request that reproduces that spend's circuit shape for the `planTransferAfter` dry run), labels, and `MERGE_SUGGEST_NOTE_COUNT` (5 — unshields / vault ops have no circuit above 4 inputs). | Working |
| `spend-fee-error.ts` | Leaf module (no imports): `SpendFeeIncreasedError`, so `lib/tx/errors` can classify it (→ `FEE_EXPIRED`, start over) without pulling in the SDK. | Working |
| `unshield-sdk.ts` | Unshield builder (`buildUnshieldSdk`: plan → reviewed-fee cap → preflight → stash → prove → `transact` calldata, returning the fee actually charged) and the unshield flows' Max, `maxUnshieldAmount` (the SDK's `wallet.maxUnshieldAmount`; a vault deposit shares it). Sibling builders: `unshield-xchain-sdk.ts`, `yield-sdk.ts`. | Working |
| `spend-fee.ts` | `totalFeeOf(plans)` (the fee a spend's plans charge — the sum of their fee notes, which can exceed the per-proof quote: several proofs, or small change folded into the fee) and `assertReviewedFee(totalFee, maxTotalFee)` — the reviewed-fee cap every spend builder (transfer, unshield, xchain unshield, yield, merge) applies before proving. | Working |
| `pending-spend.ts` | armada-sdk #55 in-flight double-spend guard: builders `stashSpendPlan(recordId, plans)`, handlers `markSpendPendingForRecord` after broadcast (all groups at once) and `clearSpendPendingForTx` on revert. Every spend handler's outer `catch` calls `forgetSpendPlan(record.id)`, so a build/submit that never broadcasts doesn't leave plans in memory. | Working |
