...
- **Single-leader execution.** Only the tab holding the `armada-tx-executor` `navigator.locks` lock runs handlers. Other tabs are passive observers. When the leader tab closes, an open follower is promoted to leader (via a queued lock request) and resumes watching in-flight txs — no reload needed (#3). There's still no follower-side *live* mirroring while a leader is active — a follower only starts driving once promoted.
